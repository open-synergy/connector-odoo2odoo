...
class OdooProductPricelistItemExporter(OdooSyncExporter):
    _model_name = "odoo.product.pricelist.item"
    ...
    def match_external_record(self, binding):
        ...
        for lang_code in lang_codes:
            ...
            logger.debug(
                u"%s - Try to match the Pricelist Item '%s' (lang='%s')...",
            ...
            if external_id:
                ...
                logger.debug(
                    u"%s - Pricelist Item '%s' (ID=%s) matches with "
                    u"the external Pricelist Item '%s' (ID=%s)",
